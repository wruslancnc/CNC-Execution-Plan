...
void AI_AsynchronousOneBufferedAi::OnStoppedEvent(void * sender, BfdAiEventArgs * args, void * userParam)
{
    ...
    do
    {
        ...
        ((WaveformAiCtrl*)sender)->GetData(args->Count, uParam->scaledData, 0, &returnedCount, NULL, NULL, NULL);
        remainingCount -= returnedCount;

        uParam->graph->Chart(uParam->scaledData, uParam->configure.channelCount, returnedCount / uParam->configure.channelCount, 1.0 / uParam->configure.clockRatePerChan);
