...
void ConfigureDialog::Initailization()
{
    ...
    Array<DeviceTreeNode> *supportedDevices = m_bfdEventCounterCtrl->getSupportedDevices();
...
void ConfigureDialog::DeviceChanged()
{
    ...
    ErrorCode errorCode = m_bfdEventCounterCtrl->setSelectedDevice(selected);
    ...
    //configure CheckBox for ChanneMap
    Max_Channel_Count = m_bfdEventCounterCtrl->getFeatures()->getChannelCountMax();
    ...
    configure.trgCount = m_bfdEventCounterCtrl->getFeatures()->getTriggerCount();
    if(configure.trgCount){
        Array<SignalDrop>* trgSrc = m_bfdEventCounterCtrl->getFeatures()->getTriggerSources();
        ...
        Array<TriggerAction>* trgAct = m_bfdEventCounterCtrl->getFeatures()->getTriggerActions();
...
{
    ui.txtSectionLength->setText(QString::number(m_bfdEventCounterCtrl->getRecord()->getSectionLength()));
    ui.txtSectionCount->setText(QString::number(m_bfdEventCounterCtrl->getRecord()->getSectionCount()));
    ...
    if(configure.trgCount){
        ...
        ui.txtTrgLevel->setText(QString::number(m_bfdEventCounterCtrl->getTrigger()->getLevel(), 'f',0.0));
    }

    m_bfdEventCounterCtrl->getChannelMap(Max_Channel_Count, configure.channelEnable);
    for(int i = 0; i < Max_Channel_Count; i++) {
