...
void ConfigureDialog::Initailization()
{
    ...
    Array<DeviceTreeNode> *supportedDevices = m_bfdPwModulatorCtrl->getSupportedDevices();
...
void ConfigureDialog::DeviceChanged()
{
    ...
    ErrorCode errorCode = m_bfdPwModulatorCtrl->setSelectedDevice(selected);
    ...
    Max_Channel_Count = m_bfdPwModulatorCtrl->getFeatures()->getChannelCountMax();
    ...
    configure.trgCount = m_bfdPwModulatorCtrl->getFeatures()->getTriggerCount();
    if(configure.trgCount){
        ...
        ui.cbxTrgSource->setEnabled(false);
...
void ConfigureDialog::ButtonOKClicked()
{
    ...
    if(ui.radBtn_LoadProfile->isChecked()){
        ...
        configure.deviceName = ui.cmbDevice->currentText();
        configure.intervalCount = ui.txtIntervalCount->text().toInt();
...
{
    ui.txtIntervalCount->setText(QString::number(m_bfdPwModulatorCtrl->getIntervalCount()));

    SignalDrop clkSrc = m_bfdPwModulatorCtrl->getSampleClkSrc();
    CounterClockSourceIndexer* indexer = m_bfdPwModulatorCtrl->getFeatures()->getPoSampleClockSources();
    for(int i = 0; i < indexer->getItem(0)->getCount(); i++){
    ...
    if(configure.trgCount){
        SignalDrop trgSrc = m_bfdPwModulatorCtrl->getTrigger()->getSource();
        Array<SignalDrop>*  sources = m_bfdPwModulatorCtrl->getFeatures()->getTriggerSources();
        ...
        TriggerAction trgAction = m_bfdPwModulatorCtrl->getTrigger()->getAction();
        Array<TriggerAction>* action = m_bfdPwModulatorCtrl->getFeatures()->getTriggerActions();
        ...
        ActiveSignal trgEdge = m_bfdPwModulatorCtrl->getTrigger()->getEdge();
        int index = -1;
        trgEdge == RisingEdge ? (index = 0) : (index = 1);
        ui.cbxTrgEdge->setCurrentIndex(index);

        ui.txtTrgLevel->setText(QString::number(m_bfdPwModulatorCtrl->getTrigger()->getLevel(), 'f',0.0));
    }

    m_bfdPwModulatorCtrl->getChannelMap(Max_Channel_Count, configure.channelEnable);
    for(int i = 0; i < Max_Channel_Count; i++) {
